...
def users():
    ...
    # Inclut les utilisateurs actifs ET ceux en attente d'abonnement
...
def convert_prospect(prospect_id):
    """
    Convertir un prospect en envoyant une invitation de création de compte par email.
    """
    if not current_user.is_admin:
        return jsonify({'success': False, 'message': 'Accès non autorisé'}), 403
    ...
    try:
        ...
        prospect.invitation_token = invitation_token
        prospect.invitation_sent_at = datetime.utcnow()
        prospect.invitation_expires_at = datetime.utcnow() + timedelta(days=7)  # Expire dans 7 jours
        
        # Construire l'URL de création de compte
        signup_url = request.url_root.rstrip('/') + '/plateforme/inscription-invitation'
        
        # Envoyer l'email d'invitation
        from app.services.email_service import mailer_send_service
        
        email_result = mailer_send_service.send_prospect_invitation(
        ...
        if email_result['success']:
            # Marquer le prospect comme ayant reçu une invitation
            prospect.prospect_status = 'contacté'  # Ou créer un nouveau statut 'invité'
            
            db.session.commit()
            
            return jsonify({
                'success': True,
                'message': 'Invitation envoyée avec succès ! Le prospect recevra un email pour créer son compte.',
            ...
            return jsonify({
                'success': False, 
                'message': f'Erreur lors de l\'envoi de l\'email: {email_result["message"]}'
            }), 500
        
    except Exception as e:
        db.session.rollback()
        print(f"Erreur lors de l'envoi d'invitation: {str(e)}")
        return jsonify({'success': False, 'message': f'Erreur lors de l\'envoi d\'invitation: {str(e)}'}), 500
