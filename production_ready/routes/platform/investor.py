...
@platform_investor_bp.route('/api/select-plan', methods=['POST'])
@login_required
def select_plan_api():
    """
    API pour marquer qu'un prospect a sélectionné un plan.
    """
    if not current_user.is_prospect_type():
        return jsonify({'error': 'Accès réservé aux prospects'}), 403
    
    data = request.get_json()
    if not data or 'plan' not in data:
        return jsonify({'error': 'Plan non spécifié'}), 400
    
    plan_type = data['plan']
    if plan_type not in ['initia', 'optima']:
        return jsonify({'error': 'Plan invalide'}), 400
    
    try:
        # Marquer le plan comme sélectionné
        current_user.plan_selected = True
        db.session.commit()
        
        return jsonify({'success': True, 'plan': plan_type})
...
def dashboard():
    ...
    if payment_success:
        flash('Paiement effectué avec succès ! Votre abonnement Atlas est maintenant actif.', 'success')
    
    if not current_user.subscription or not current_user.subscription.is_active():
        if current_user.is_prospect_type():
            # Prospect connecté sans abonnement -> rediriger vers choix de plans
            return redirect(url_for('platform_auth.choose_plan'))
        else:
            # Utilisateur existant dont l'abonnement a expiré
            return redirect(url_for('platform_auth.login'))
    
    # Pour les prospects sans profil investisseur, on affiche quand même le dashboard avec popup
    if not current_user.investor_profile and not current_user.is_prospect_type():
        return redirect(url_for('platform_investor.questionnaire'))
    
    # Valeurs par défaut pour les prospects sans profil
    ...
    # Si l'utilisateur a un profil investisseur, utiliser ses vraies données
