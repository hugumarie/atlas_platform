...
class User(UserMixin, db.Model):
    ...
    password_hash = db.Column(db.String(255), nullable=True)  # Nullable pour les prospects
    ...
    # Contrôle du parcours prospect
    plan_selected = db.Column(db.Boolean, default=False, nullable=False)  # True si le prospect a choisi un plan
    payment_completed = db.Column(db.Boolean, default=False, nullable=False)  # True si le paiement a été effectué
    ...
    def mark_as_converted(self):
        """
        Marque le prospect comme converti en client SEULEMENT après paiement.
        Cette méthode ne doit être appelée que quand le prospect a un abonnement payant actif.
        """
        if self.is_prospect_type() and self.subscription and self.subscription.is_active():
            ...
            # Créer le profil investisseur maintenant qu'il est client payant
            if not self.investor_profile:
                ...
                investor_profile = InvestorProfile(
                    ...
                    investment_goals='',  # Laisser vide pour éviter les contraintes
